...
@else
    <form action="{{ url('/transaksi/' . $transaksi->penjualan_id . '/update_ajax') }}" method="POST" id="form-edit">
                            ...
                            <ul class="todo-list" data-widget="todo-list" id="transaction-detail-list">
                                @foreach ($transaksi->transaksiDetail as $detail)
                                    <li id="detail-{{ $detail->detail_id }}">
                                        ...
                                        <button type="button" class="btn btn-info btn-sm float-right edit-detail"
                                            data-id="{{ $detail->detail_id }}" data-barang-id="{{ $detail->barang_id }}"
                                            data-jumlah="{{ $detail->jumlah }}" data-harga="{{ $detail->harga }}">Edit</button>
        ...
        <!-- Modal for editing detail transaction -->
    <div id="modal-edit-detail" class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header">
            ...
            <div class="modal-body">
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="edit_jumlah" id="edit_jumlah" class="form-control" min="1">
                    <small id="error-edit_jumlah" class="error-text form-text text-danger"></small>
                </div>
                <div class="form-group">
                    <label for="edit_harga">Harga</label>
                    <input type="number" name="edit_harga" id="edit_harga" class="form-control" min="0">
    ...
    <script>
        $(document).ready(function() {
            ...
            $("#update-detail").on("click", function() {
                ...
                $("#detail-" + detailId + " small.badge-success").text("Harga: " + new Intl.NumberFormat().format(newHarga));
